...
def find_similar_item(user_to_rating):
    ...
    return find_similar_item_cf(item_to_vector)
...
def item_cf(user_id, item_id, similar_items, item_to_vector, topn=10):
    ...
    pred_score = 0
    count = 0
    for similar_item, similarity in similar_items[item_id - 1][:topn]:
        #相似用户对这部电影的打分
        rating_by_similiar_item = item_to_vector[user_id][similar_item - 1]
        #分数*用户相似度，作为一种对分数的加权，越相似的用户评分越重要
